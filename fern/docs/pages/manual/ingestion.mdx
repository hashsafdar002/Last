...
When you are running DocuChat in a fully local setup, you can ingest a complete folder for convenience (containing
pdf, text files, etc.)
...
DocuChat by default supports all the file formats that contains clear text (for example, `.txt` files, `.html`, etc.).
...
**Please note the following nuance**: while `DocuChat` supports these file formats, it **might** require additional
dependencies to be installed in your python's virtual environment.
For example, if you try to ingest `.epub` files, `DocuChat` might fail to do it, and will instead display an
