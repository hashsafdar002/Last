...
DocuChat supports running with different LLMs & setups.
...
decide to run DocuChat using OpenAI as the LLM and Embeddings model.
...
And run DocuChat loading that profile you just created:

`PGPT_PROFILES=openai make run`

or

`PGPT_PROFILES=openai poetry run python -m docu_chat`
...
`PGPT_PROFILES=sagemaker poetry run python -m docu_chat`
